...
def lay_uoc_luong(items : List[Item], capacity :int, index : int):
    uoc_luong = 0.0 
    for i in range(index, len(items)):
        item = items[i]
        if capacity >= item.weight:
            uoc_luong += item.value
            capacity -= item.weight
        else:
            uoc_luong += float(item.value) * capacity / item.weight
            break
    return uoc_luong
...
def search(items : List[Item], capacity : int):
    ...
    start_expectation = lay_uoc_luong(items, capacity, 0)
    ...
    while(len(stack) != 0):
        ...
        notake_expectation = notake_value + lay_uoc_luong(items, notake_capacity, cur_pos + 1)
        ...
        take_expectation = take_value + lay_uoc_luong(items, take_capacity, cur_pos + 1)
